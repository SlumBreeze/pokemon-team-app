...
const STAT_KEYS: (keyof StatSpread)[] = [
  "hp",
  "attack",
  "defense",
  "specialAttack",
...
const StatEditor: React.FC<StatEditorProps> = ({ member, onUpdate }) => {
  ...
  const getBaseStat = (statKey: keyof StatSpread): number => {
    ...
    const apiKey = Object.entries(API_STAT_NAMES).find(
      ([_, v]) => v === statKey
    )?.[0];
    const stat = (member.data.stats || []).find(
      (s: PokemonStat) => s.stat.name === apiKey
    );
  ...
  const getFinalStat = (statKey: keyof StatSpread): number => {
    ...
    const natureMultiplier = isHp
      ? 1
      : getNatureMultiplier(nature, statKey as any);
      ...
      {isExpanded && (
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <label className="text-[10px] text-gray-500 dark:text-dark-text-secondary uppercase font-bold">
              Nature:
            </label>
            <select
              value={nature}
            ...
            >
              {Object.keys(NATURES).map((n) => {
                ...
                if (effect.up && effect.down) {
                  label += ` (+${STAT_LABELS[effect.up]} -${
                    STAT_LABELS[effect.down]
                  })`;
                }
                return (
          ...
          <div className="grid gap-2">
            {STAT_KEYS.map((statKey) => {
              ...
              return (
                <div
                  key={statKey}
                  className="flex items-center gap-2 bg-white dark:bg-dark-border/50 rounded-lg p-2 border border-black/5 dark:border-dark-border"
                >
                  {/* Stat Label */}
                  <div
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      value={ivs[statKey]}
                      onChange={(e) =>
                        updateIv(statKey, parseInt(e.target.value) || 0)
                      }
                  ...
                  <div className="flex-1 flex items-center gap-1">
                    ...
                    <input
                      ...
                      value={evs[statKey]}
                      onChange={(e) =>
                        updateEv(statKey, parseInt(e.target.value))
                      }
                    ...
                    <input
                      ...
                      value={evs[statKey]}
                      onChange={(e) =>
                        updateEv(statKey, parseInt(e.target.value) || 0)
                      }
                      className="w-10 bg-gray-100 dark:bg-dark-card border border-black/10 dark:border-dark-border rounded px-1 py-0.5 text-[10px] font-mono text-center focus:outline-none focus:ring-1 focus:ring-purple-500"
                    />
                  </div>
